                                ...
                                <div class="card">
                                    ...
                                    <div class="card-block table-border-style">
                                        ...
                                        if (isset($_REQUEST["btnsave"])) {
                                            ...
                                            $items_id = mysqli_real_escape_string($link, $_REQUEST['items_id']);
                                            ...
                                            $innst = "insert into tbl_sales(user_id,client_id,cust_type,sale_date,cust_name,cust_mobile,cust_addr,items_id,ttl_quty,ttl_amt,recv,discount,balance,payment_mode,disc) values('$userid','$client_id','$cust_type','$sale_date','$cust_name','$cust_mobile','$cust_addr','$codecode','$ttl_quty','$ttl_amt','$recv','$discount','$balance','$payment_mode','$disc')";
                                            ...
                                            for ($i = 0; $i < $count; $i++) {
                                                $query = "INSERT INTO `tbl_items` (`user_id`,`sale_id`,`rand_id`,`item_type`,`item_name`,`qty`,`price`,`amount`) VALUES('{$_POST[user_id][$i]}','$sale_id','$codecode','{$_POST[item_type][$i]}','{$_POST[item_name][$i]}','{$_POST[qty][$i]}','{$_POST[price][$i]}','{$_POST[amount][$i]}')";
                                                //echo $query;
                                                $resultt = mysqli_query($link, $query);
                                            }

                                            //$update_id="UPDATE tbl_sales SET items_id={$_POST[id][$i]} where id='$last_id'";
                                            if (mysqli_affected_rows($link)) {

                                                if ($recv > 0) {
                                                    $insert_daybook = mysqli_query($link, "insert into tbl_client_daybook(client_id, amount, trans_type, pay_date, remarks, addon) values('$client_id','$recv','cr','$sale_date','Add Sale',now())");
                                                }
                                                    ...
                                                    <div class="table-responsive">
                                                        <table class="table" id="tbl_posts">
                                                            ...
                                                            <tbody id="tbl_posts_body">
                                                                <tr id="rec-1" class="txtMult">
                                                                    <td>
                                                                        <div class="form-group basic" style="padding: 0px;">
                                                                            <div class="input-wrapper">
                                                                                <select class="form-control item_input_in_table custom-select" name="item_name[]">
                                                                                    <option value="Select">Select Item</option>
                                                                                    <?php
                                                                                    $q = mysqli_query($link, "SELECT ap.frei_id, ap.pro_id, p.pro_name from tbl_assign_products as ap INNER JOIN tbl_product as p on ap.pro_id=p.id WHERE frei_id=$userid");
                                                                                    while ($datac = mysqli_fetch_array($q)) {
                                                                                    ?>
                                                                                        <option value="<?= $datac['id']; ?>"><?= $datac['pro_name']; ?></option>
                                                                                    <?php
                                                                                    }
                                                                                    ?>
                                                                                </select>
                                                                                <input type="text" name="item_type[]" value="sale" style="display: none;">
                                                                    ...
                                                                    <td>
                                                                        <div class="form-group basic" style="padding: 0px;">
                                                                            <div class="input-wrapper">
                                                                                <input type="tel" name="qty[]" class="form-control item_input_in_table val1">
                                                                            </div>
                                                                        </div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="form-group basic" style="padding: 0px;">
                                                                            <div class="input-wrapper">
                                                                                <input type="tel" name="price[]" class="form-control item_input_in_table val2">
                                                                            </div>
                                                                        </div>
                                                                    </td>
                                                                    <td>
                                                                        <div class="form-group basic" style="padding: 0px;">
                                                                            <div class="input-wrapper">
                                                                                <input type="tel" name="amount[]" id="every_click" class="form-control item_input_in_table multTotal">
                                                                            </div>
                                                                        </div>
                                                                    </td>
                                                                    <td>
                                                                        <a class="btn btn-danger btn-sm delete-record btn-radius" data-id="0">
                                                                            X
                                                                        </a>
                                                                    </td>
                                                                </tr>
                                                            ...
                                                            <tfoot>
                                                                <tr>
                                                                    <td colspan="2" style="padding: 10px 0px;">
                                                                        <a class="btn btn-success btn-sm add-record" data-added="0">
                                                                            Add Item
                                                                        </a>
                                                                    </td>
                                                                    <td colspan="3" class="right">
                                                                        Subtotal: &nbsp;<span class="grandTotal"></span>
                                                                    </td>
                                                                </tr>
                                                            </tfoot>
                                                        </table>
    ...
    <table id="sample_table">
        <tr id="" class="txtMult">
            <td>
                <div class="form-group basic" style="padding: 0px;">
                    <div class="input-wrapper">
                        ...
                        <input type="text" name="item_type[]" value="sale" style="display: none;">
                        <input type="text" name="user_id[]" value="<?= $userid; ?>" style="display: none;">
                        <!-- <input type="text" name="sale_id[]" value="<?= $dataq['id']; ?>" style="display: none;"> -->
                    </div>
                </div>
            </td>
            <td>
            ...
            <td>
                <div class="form-group basic" style="padding: 0px;">
                    <div class="input-wrapper">
                        <input type="tel" name="amount[]" class="form-control item_input_in_table multTotal">
                    </div>
                </div>
            </td>
            <td>
                <a class="btn btn-light btn-sm delete-record" data-id="0">
    ...
    function getdata(tid) {
        $.ajax({
            ...
            success: function (data) {
                $('#vvvvv').val(data[0].rate);
                $('#inc_rate').val(data[0].inct);
                $("#e1w").html("");
                // $("#e1w").append("<option value='' > Select Customer </option> ");
                // $.each(data, function (i,val) {
                //     var result = '<option value="' + val.cid + '">' + val.cust_id + ' ( ' + val.cust_name + ' )</option>';
                //     $("#e1w").append(result);
                // });
            }
        });
